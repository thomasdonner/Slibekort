...
import { hentHusketNavn, gemHusketNavn } from "@/lib/klient/husket-navn";
...
type Skaerm =
  ...
  | {
      ...
      saldo: number;
      tidspunkt: number;
      // Kun sat på en delt konto — skal bruges uændret, hvis der fortrydes,
      // uden at spørge sliberen om navnet igen.
      udfoertAfNavn: string | null;
    }
  | { navn: "fejl"; besked: string };

type Fane = "scan" | "vaelg";

export default function SlibKlient({ delt }: { delt: boolean }) {
  ...
  const slaOp = useCallback(
    async (raaScan: string) => {
      ...
      setNavnFejl(null);
      // Husket i en time (lib/klient/husket-navn.ts) — sliberen skal
      // normalt bare bekræfte, ikke skrive navnet igen for hver spiller.
      setUdfoertAfNavn(delt ? hentHusketNavn() : "");
  ...
  async function traek(tvangstraek: boolean) {
    ...
    let gemtNavn: string | null = null;
    if (delt) {
      if (!erGyldigtUdfoertAfNavn(udfoertAfNavn)) {
        setNavnFejl("Skriv dit navn — kontoen er delt.");
        return;
      }
      gemtNavn = udfoertAfNavn.trim();
      gemHusketNavn(gemtNavn);
    }
    ...
    const { klientId, saldo } = await traekSlibning({
      ...
      tvangstraek,
      udfoertAfNavn: gemtNavn ?? undefined,
    });
    setSkaerm({
      navn: "kvittering",
      data,
  ...
  async function fortryd() {
    ...
    await fortrydSlibning({
      ...
      udfoertAfNavn: skaerm.udfoertAfNavn ?? undefined,
    });
    ...
    <main>
      ...
      {skaerm.navn === "bekraeft" && (
        <Bekraeftelsesskaerm
          data={skaerm.data}
          delt={delt}
          udfoertAfNavn={udfoertAfNavn}
          onUdfoertAfNavnAendret={(vaerdi) => {
            setUdfoertAfNavn(vaerdi);
            setNavnFejl(null);
          }}
          navnFejl={navnFejl}
          onTraek={() => traek(false)}
...
// et smalt kort er der ikke plads nok til det — et hårdt mellemrum
...
function Bekraeftelsesskaerm({
  ...
  onUdfoertAfNavnAendret: (vaerdi: string) => void;
  navnFejl: string | null;
  onTraek: () => void;
